...
struct rocksdb_compactionfilter_t : public CompactionFilter {
  ...
  const char* Name() const override { DBUG_TRACE; return (*name_)(state_); }

  bool IgnoreSnapshots() const override { DBUG_TRACE; return ignore_snapshots_; }
};
...
void rocksdb_wal_iter_next(rocksdb_wal_iterator_t* iter) { DBUG_TRACE; iter->rep->Next(); }
...
void rocksdb_iter_next(rocksdb_iterator_t* iter) { DBUG_TRACE; iter->rep->Next(); }

void rocksdb_iter_prev(rocksdb_iterator_t* iter) { DBUG_TRACE; iter->rep->Prev(); }
...
void rocksdb_writebatch_destroy(rocksdb_writebatch_t* b) { DBUG_TRACE; delete b; }

void rocksdb_writebatch_clear(rocksdb_writebatch_t* b) { DBUG_TRACE; b->rep.Clear(); }

int rocksdb_writebatch_count(rocksdb_writebatch_t* b) { DBUG_TRACE; return b->rep.Count(); }
...
rocksdb_options_t* rocksdb_options_create() { DBUG_TRACE; return new rocksdb_options_t; }

void rocksdb_options_destroy(rocksdb_options_t* options) { DBUG_TRACE; delete options; }
...
void rocksdb_logger_destroy(rocksdb_logger_t* logger) { DBUG_TRACE; delete logger; }
...
void rocksdb_comparator_destroy(rocksdb_comparator_t* cmp) { DBUG_TRACE; delete cmp; }
...
rocksdb_filterpolicy_t* rocksdb_filterpolicy_create_bloom_format(
  ...
  struct Wrapper : public rocksdb_filterpolicy_t {
    ...
    const char* Name() const override { DBUG_TRACE; return rep_->Name(); }
    ...
    static void DoNothing(void*) {DBUG_TRACE;}
  };
...
void rocksdb_readoptions_destroy(rocksdb_readoptions_t* opt) { DBUG_TRACE; delete opt; }
...
void rocksdb_writeoptions_destroy(rocksdb_writeoptions_t* opt) { DBUG_TRACE; delete opt; }
...
void rocksdb_flushoptions_destroy(rocksdb_flushoptions_t* opt) { DBUG_TRACE; delete opt; }
...
void rocksdb_cache_destroy(rocksdb_cache_t* cache) { DBUG_TRACE; delete cache; }
...
void rocksdb_dbpath_destroy(rocksdb_dbpath_t* dbpath) { DBUG_TRACE; delete dbpath; }
...
void rocksdb_envoptions_destroy(rocksdb_envoptions_t* opt) { DBUG_TRACE; delete opt; }
...
void rocksdb_slicetransform_destroy(rocksdb_slicetransform_t* st) { DBUG_TRACE; delete st; }

struct SliceTransformWrapper : public rocksdb_slicetransform_t {
  const SliceTransform* rep_;
  ~SliceTransformWrapper() override { delete rep_; }
  const char* Name() const override { DBUG_TRACE; return rep_->Name(); }
  std::string GetId() const override { DBUG_TRACE; return rep_->GetId(); }
  Slice Transform(const Slice& src) const override {
    DBUG_TRACE;
    return rep_->Transform(src);
  }
  bool InDomain(const Slice& src) const override { DBUG_TRACE; return rep_->InDomain(src); }
  bool InRange(const Slice& src) const override { DBUG_TRACE; return rep_->InRange(src); }
  static void DoNothing(void*) {DBUG_TRACE;}
};
...
void rocksdb_livefiles_destroy(const rocksdb_livefiles_t* lf) { DBUG_TRACE; delete lf; }
...
void rocksdb_transactiondb_close_base_db(rocksdb_t* base_db) { DBUG_TRACE; delete base_db; }
...
void rocksdb_free(void* ptr) { DBUG_TRACE; free(ptr); }
...
void rocksdb_pinnableslice_destroy(rocksdb_pinnableslice_t* v) { DBUG_TRACE; delete v; }
